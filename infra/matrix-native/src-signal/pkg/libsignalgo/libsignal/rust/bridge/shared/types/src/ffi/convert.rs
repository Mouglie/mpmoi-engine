...
impl<T: IntoFfiError> ResultTypeInfo for Option<BulkPolledStreamTerminationReason<T>> {
    ...
    fn convert_into(self) -> SignalFfiResult<Self::ResultType> {
        let raw = match self {
            ...
            // mpmoi win-x64 patch: libc::MAP_FAILED is Unix-only, so it breaks the
            // windows-gnu build. MAP_FAILED is just the (void*)-1 sentinel (see the doc
            // comment above) and is never dereferenced, so use that pointer value directly
            // — portable across all targets, semantically identical.
